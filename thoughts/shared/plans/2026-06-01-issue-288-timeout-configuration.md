...
1. **Config model** (`src/vip/config.py`) — a new `TimeoutConfig` dataclass holds semantic fields in seconds (`http_request`, `connect_deploy`, `connect_task_wait`, `connect_system_check_wait`, `workbench_page_load`, `workbench_dialog`, `workbench_session_start`, `workbench_ide_load`, `workbench_code_exec`, `workbench_session_capacity_quick`, `workbench_session_capacity_cleanup`, `playwright_oidc_complete`, `playwright_mfa_complete`, `job_active_deadline`, `job_poll_wait`, `tcp_probe`, `download`). The dataclass exposes `scale: float = 1.0` and a helper `effective(field) -> float` that returns `getattr(self, field) * scale`. Loaded via `TimeoutConfig.from_dict` from a new `[timeouts]` table in `vip.toml`. `VIPConfig` gains a `timeouts: TimeoutConfig` attribute.

2. **Bridge to existing per-section knobs** — `ConnectConfig.deploy_timeout`, `WorkbenchConfig.job_timeout`, and `PerformanceConfig.page_load_timeout` / `download_timeout` stay as fields, but their `from_dict` resolution becomes: explicit value in the section → matching `[timeouts]` value → built-in default. This means the new `[timeouts]` table is the central place to set values, while old `vip.toml` files (with `deploy_timeout` under `[connect]` or `job_timeout` under `[workbench]`) keep working unchanged.
...
- `selftests/test_timeouts.py` — covers (1) defaults, (2) per-field override via TOML, (3) `scale=2.0` doubles every effective value, (4) `VIP_TIMEOUT_SCALE` env var beats the file value, (5) `connect.deploy_timeout` wins over `timeouts.connect_deploy`, (6) `workbench.job_timeout` wins over `timeouts.workbench_job_timeout`, (7) `job_active_deadline` and `job_poll_wait` are distinct fields with distinct defaults (840 s and 900 s respectively), (8) unknown `[timeouts]` keys raise `ConfigError`.
...
- `src/vip/auth.py` — replace the operation timeouts (3_000 ms click, 10_000 / 15_000 ms locator waits, the 300 s and 120 s deadlines, the 5_000 ms networkidle wait, the 10.0 s httpx waits) with `timeouts.effective(...)` reads. The two `page.wait_for_timeout(500)` poll-sleep calls in `start_interactive_auth` and `_wait_for_product_redirect` are **not** operation timeouts — they are deliberate UI pacing delays that prevent busy-polling during the browser login loop. Scaling them with `VIP_TIMEOUT_SCALE` would break the flow on fast machines by unnecessarily slowing down the poll interval; leave them as-is.
...
- `src/vip/verify/job.py` — `job.py` has two distinct timeouts that must be controlled separately: `timeout_seconds` (the Kubernetes Job `activeDeadlineSeconds`, default 840 s, passed to `create_job`) and the log-stream/poll wait in `stream_logs` / `wait_for_job` (default 900 s). Replace them with `timeouts.effective("job_active_deadline")` and `timeouts.effective("job_poll_wait")` respectively.
- `src/vip_tests/conftest.py` — expose a `timeouts` fixture that returns `vip_config.timeouts` for step files to consume.
- `src/vip_tests/workbench/conftest.py` — the module-level millisecond constants (`TIMEOUT_QUICK`, `TIMEOUT_DIALOG`, `TIMEOUT_PAGE_LOAD`, `TIMEOUT_IDE_LOAD`, `TIMEOUT_SESSION_START`, `TIMEOUT_CLEANUP`, `TIMEOUT_CODE_EXEC`) **cannot** be replaced by fixture-derived values, because step files import them directly at module load time (before pytest fixtures exist). Two acceptable strategies: (a) keep the constants as static defaults and pass the `timeouts` fixture explicitly through to the helper functions that consume them (e.g. add an optional `timeout` parameter to `wait_for_session_active`, `assert_homepage_loaded`, etc.) so fixture-aware callers can override them; or (b) keep the constants as unchangeable defaults and document them as a known gap — operators who need to tune these values must edit the source directly. Strategy (a) is preferred: it preserves backward compatibility for callers that omit the parameter while letting fixture-aware step definitions pass `vip_config.timeouts.effective("workbench_session_start") * 1000` where needed. Step files that do not pass an explicit timeout continue to use the module-level constant.
