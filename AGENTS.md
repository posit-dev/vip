...
-   **`linux-smoke.yml`** -- push to `main` or PR (paths-gated to `docker/rhel*/`, `docker/opensuse-leap/`, `justfile`, `pyproject.toml`, `uv.lock`, `src/**`, `selftests/**`); builds a Docker image per distro (`rhel9`, `rhel10`, `opensuse-leap`) that exercises `vip install`'s `apt`/`zypper` system-package path, then runs the smoke script inside it.
