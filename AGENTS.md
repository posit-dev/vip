...
| `src/vip/plugin.py` | pytest plugin: markers (including `slow`, used by `verify --basic`), auto-skip, JSON report output; `pytest_configure` also registers `vip.fixtures` as its own named plugin (`"vip-fixtures"`) so core fixtures resolve regardless of directory ancestry — see that module's docstring. `pytest_bdd_apply_tag` turns each `@control-<slug>` Gherkin tag into one `control(<slug>)` mark (every other tag falls through to pytest-bdd's default), so a customer's own slugs need no registering by name and are not bound by pytest's marker-naming rules; `_control_marker_names` writes the slug back out as `control-<slug>` for `results.json`, which is what `vip.traceability` joins against |
