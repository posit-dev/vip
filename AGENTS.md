...
-   `recent-production-versions.yml` -- `workflow_call` only; the single source of product versions for CI. Queries a public `ghcr.io/posit-dev/*` repository's tag list anonymously (no `docker login`, no `packages: read` -- and note that `example-report.yml` must not request `packages` either, since a called workflow may only reduce its caller's permissions and none of its three callers grant that scope; it declares `contents: read`, a reduction of the `contents: write` all three grant, rather than omitting the block and inheriting a write token). Emits `latest` and a `versions` JSON array holding the newest release tag from each of the last `count` calendar months. One tag per month is what guarantees no resolved version is a substring of another, which the job-rename steps in the smoke workflows rely on to find their own check run by name.
-   **Smoke workflows** (`connect-smoke.yml`, `workbench-smoke.yml`, `packagemanager-smoke.yml`, `mock-idp-e2e.yml`) -- run the product suites against real containers. On PR/push each tests a single latest version (change-gated via a `changes` paths-filter job); on `schedule` (nightly, staggered hourly) and `workflow_dispatch` a `set-matrix` job fans each out across the product version support window (current + 2 back). There are no pinned tags to bump: every `set-matrix` job calls `recent-production-versions.yml`, which resolves versions from the product's own GHCR tag list at run time. Each workflow's `*-status` aggregation job is scope-aware: it passes when the suite was legitimately out of scope (the PR's paths didn't match) but **fails** when the suite was in scope (`changes.relevant == 'true'`) yet did not succeed -- so a path-gated skip, an excluded actor, or a missing license secret cannot report a green required check without the suite having run. The Connect, Workbench, and Package Manager `*-status` jobs are required merge checks.
...
-   **`connect-integration.yml`** -- push to `main`, `schedule` (daily), and `workflow_dispatch`; deliberately no `pull_request` trigger and not a required check (decided in #421). Runs the full VIP Connect category (`vip verify --api-auth --categories connect`) against two ephemeral Connect servers via `with-connect`: the current production release (resolved by `recent-production-versions.yml`) and `ghcr.io/posit-dev/connect-preview:daily`, the nightly build, so an unreleased upstream build never blocks a PR. It's a drift detector, not a gate: failures post to Slack via the `SLACK_WEBHOOK_CONNECT_CI` secret instead of failing anyone's merge.
