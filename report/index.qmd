...
[Summary](index.html) · [Detailed Results](details.html)
...
.vip-fail-error {
  background: #1e1e1e; color: #d4d4d4; font-size: 0.8125rem;
  line-height: 1.5; padding: 1rem; border-radius: 4px;
...
.vip-fail-hints ul, .vip-fail-hints ol { margin: 0.25rem 0 0.75rem 1.25rem; padding: 0; }
...
""")
    ...
    for outcome in _OUTCOME_ORDER:
        ...
        for tag in sorted(product_groups.keys()):
            ...
            for item in items:
                ...
                # Error traceback + troubleshooting hints (failed only)
                error_html = ""
                hints_html = ""
                if item.outcome == "failed":
                    if item.longrepr:
                        eid = f"vip-error-{_error_idx}"
                        _error_idx += 1
                        error_html = (
                            f'<details class="vip-fail-details">'
                            f"<summary>Error traceback</summary>"
                            f'<div class="vip-fail-error-wrap">'
                            f'<button class="vip-copy-btn" data-target="{eid}"'
                            f' title="Copy to clipboard">Copy</button>'
                            f'<pre id="{eid}" class="vip-fail-error">'
                            f"{_esc(item.longrepr)}</pre>"
                            f"</div></details>"
                        )
                    hint = _hints.get(item.scenario_title, {}) if item.scenario_title else {}
                    if hint:
                        _hp = []
                        if hint.get("likely_causes"):
                            _hp.append("<strong>Likely causes:</strong><ul>")
                            for c in hint["likely_causes"]:
                                _hp.append(f"<li>{_esc(c)}</li>")
                            _hp.append("</ul>")
                        if hint.get("suggested_steps"):
                            _hp.append("<strong>Suggested next steps:</strong><ol>")
                            for s in hint["suggested_steps"]:
                                _hp.append(f"<li>{_esc(s)}</li>")
                            _hp.append("</ol>")
                        if hint.get("docs_url"):
                            _u = _esc(hint["docs_url"])
                            _hp.append(
                                f'<p><strong>Documentation:</strong> '
                                f'<a href="{_u}" target="_blank" rel="noopener">{_u}</a></p>'
                            )
                        if _hp:
    ...
    # Copy-to-clipboard script for error tracebacks
    _html_parts.append("""
<script>
...
if not checks_path.exists():
    ...
    # Render as raw HTML rather than a large Markdown pipe table.
    # Pandoc's pipe-table reader hangs on tables with many rows that
    # contain long/pre-escaped cells (observed >3min on ~300 rows in CI);
    # raw HTML is passed through verbatim.
    _rows_html = []
    for r in results:
        group = html.escape(str(r.get("group", {}).get("name", "") or ""))
        test = html.escape(str(r.get("test", {}).get("name", "") or ""))
        status = "✅" if r.get("passed") else "❌"
        raw_output = str(r.get("output", "") or "").strip()
        raw_error = str(r.get("error", "") or "").strip()
        raw_detail = raw_error if raw_error else raw_output
        detail = html.escape(raw_detail).replace("\n", "<br>")
        _rows_html.append(
            f"<tr><td>{status}</td><td>{group}</td>"
            f"<td>{test}</td><td>{detail}</td></tr>"
        )

    display(HTML(
        "<table><thead><tr><th></th><th>Group</th>"
        "<th>Check</th><th>Detail</th></tr></thead><tbody>"
        + "".join(_rows_html)
        + "</tbody></table>"
    ))
```
