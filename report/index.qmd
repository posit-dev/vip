...
""")
    ...
    for cat, items in sorted(categories.items()):
        ...
        for item in items:
            ...
            outcome_css = _OUTCOME_CSS.get(item.outcome, "skipped")
            raw_title = item.scenario_title or (
                item.nodeid.split("::")[-1] if "::" in item.nodeid else item.nodeid
            )
            # Substitute <param> placeholders with pytest parametrize values from nodeid
            # e.g., "<product> server is reachable" + nodeid ending in "[Connect]" → "Connect server is reachable"
            import re as _re
            ...
            _html_parts.append(
                f'<div class="vip-test-card" style="border-left-color:{"#16a34a" if outcome_css == "passed" else "#dc2626" if outcome_css == "failed" else "#d1d5db"}">'
