...
if not results_path.exists():
    Markdown("⚠ No results.json found. Run: `pytest --vip-report=report/results.json`")
else:
    ...
    for cat, items in sorted(categories.items()):
        ...
        lines.append("|---|---|---|")
        for item in items:
            # Shorten the nodeid for display.
            short = item["nodeid"].split("::")[-1] if "::" in item["nodeid"] else item["nodeid"]
            duration = f"{item.get('duration', 0):.2f}s"
            icon = {"passed": "PASS", "failed": "FAIL", "skipped": "SKIP"}.get(item["outcome"], item["outcome"])
