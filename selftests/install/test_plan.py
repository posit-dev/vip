...
def test_uninstall_plan_emits_dnf_command():
    plan = pl.build_uninstall_plan(manifest=_full_manifest(), connect_url=None)
    assert plan.system_remove_commands == ("sudo dnf remove libdrm nss",)


def test_uninstall_plan_emits_apt_when_manifest_apt():
    m = _full_manifest()
