    ...
    fake = FakeRun(
        {
            ...
            ("dpkg-query", "-W", "-f=${Package}\t${Status}\t${Provides}\n"): _ok(
                "some-other-package\tinstall ok installed\tsome-other-name\n"
            ),
        }
    )
    monkeypatch.setattr(pkg.subprocess, "run", fake)
    result = pkg.installed_dpkg(("libnss3", "libdrm2", "libcups2"))
    assert result == {"libnss3": "libnss3", "libcups2": "libcups2"}
...
# not a hand-maintained rename table. `installed_dpkg` returns the concrete
# (real) package name that satisfies each requested name, not just whether it's
# satisfied, so callers that record installation state (e.g. for `vip uninstall`)
# record what's actually removable rather than an alias apt/dpkg don't recognize.

_BULK_QUERY = ("dpkg-query", "-W", "-f=${Package}\t${Status}\t${Provides}\n")
...
def test_installed_dpkg_detects_real_package_installed(monkeypatch):
    ...
    assert result == {"libcups2": "libcups2"}
    ...
    fake = FakeRun(
        {
            ...
            _BULK_QUERY: _ok("libcups2t64\tinstall ok installed\tlibcups2\n"),
        }
    )
    monkeypatch.setattr(pkg.subprocess, "run", fake)
    result = pkg.installed_dpkg(("libcups2",))
    assert result == {"libcups2": "libcups2t64"}
...
def test_installed_dpkg_absent_package_not_detected(monkeypatch):
    ...
    fake = FakeRun(
        {
            ("dpkg-query", "-W", "-f=${Status}", "libnonexistent9"): _not_found(),
            _BULK_QUERY: _ok("libcups2t64\tinstall ok installed\tlibcups2\n"),
        }
    )
    monkeypatch.setattr(pkg.subprocess, "run", fake)
...
def test_installed_dpkg_provides_with_version_qualifier(monkeypatch):
    ...
    fake = FakeRun(
        {
            ("dpkg-query", "-W", "-f=${Status}", "libcups2"): _not_found(),
            _BULK_QUERY: _ok("libcups2t64\tinstall ok installed\tlibcups2 (= 2.4.7)\n"),
        }
    )
    monkeypatch.setattr(pkg.subprocess, "run", fake)
    result = pkg.installed_dpkg(("libcups2",))
...
def test_installed_dpkg_provides_multiple_names(monkeypatch):
    ...
    fake = FakeRun(
        {
            ("dpkg-query", "-W", "-f=${Status}", "libcups2"): _not_found(),
            ("dpkg-query", "-W", "-f=${Status}", "libcups-2"): _not_found(),
            _BULK_QUERY: _ok("libcups2t64\tinstall ok installed\tlibcups2, libcups-2\n"),
        }
    )
    monkeypatch.setattr(pkg.subprocess, "run", fake)
    result = pkg.installed_dpkg(("libcups2", "libcups-2"))
    assert result == {"libcups2": "libcups2t64", "libcups-2": "libcups2t64"}
    ...
    fake = FakeRun(
        {
            ...
            _BULK_QUERY: _ok("libcups2t64\tdeinstall ok config-files\tlibcups2\n"),
        }
    )
    monkeypatch.setattr(pkg.subprocess, "run", fake)
    result = pkg.installed_dpkg(("libcups2",))
